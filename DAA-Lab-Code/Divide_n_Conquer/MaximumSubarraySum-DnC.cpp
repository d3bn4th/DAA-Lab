...
int MaxSubArraySum(int A[], int low, int high) { // T(n)
    if(low == high) return A[low]; // Base Condition
    // Divide
    int mid = (low + high)/2; // 1
    int LSum =  MaxSubArraySum(A,low, mid); // T(n/2)
    int RSum = MaxSubArraySum(A, mid + 1, high); // T(n/2)
    int CSum = MaxCrossSum(A,low,mid,high); // iterative n
